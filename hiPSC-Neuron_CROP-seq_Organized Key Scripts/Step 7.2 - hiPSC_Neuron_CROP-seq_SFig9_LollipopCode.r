...
# Filter cropseqdat based on variants_of_interest
filtered_data <- cropseqdat_adj[cropseqdat_adj$rsid %in% variants_of_interest, ]

# Apply FDR correction using the Benjamini-Hochberg method
filtered_data$glm_FDR <- p.adjust(filtered_data$glm_p_left_sided, method = "BH")

# Create a new column 'Significance'
filtered_data$Significance <- ifelse(filtered_data$glm_FDR < 0.05, TRUE, FALSE) #Use if using permuted dataframe glm p

#Include the 'start' values from geneAnno onto cropseqdat based on gene name/symbol
cropseqdat_genepos <- left_join(filtered_data, geneAnno %>% 
                                  dplyr::select(hgnc_symbol, ensembl_gene_id, start), 
                                by = c("gene" = "hgnc_symbol"))
#Save the data
# file_name <- paste0("/proj/hyejunglab/cropseq/Alejandro/CD_CROP-Seq_AG/CD_CROP-Seq_REAL_4-4-24/Cell_Rebuttal+Publication_Analyses/Publication_FinalPermutations/Data/cropseqdat_subset_", gene_of_interest, ".csv")
# write.csv(filtered_data, file_name, row.names = FALSE)

#Now filter for only the gene of interest
genecentered_data <- filtered_data[filtered_data$gene == gene_of_interest & filtered_data$rsid %in% variants_of_interest, ]
